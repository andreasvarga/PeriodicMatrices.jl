...
The function `phess` is based on a wrapper for the SLICOT subroutine `MB03VW`
   (see [`PeriodicMatrices.SLICOTtools.mb03vw!`](@ref)).
   
The function `phess1` is based on wrappers for the SLICOT subroutines `MB03VD`  
(see [`PeriodicMatrices.SLICOTtools.mb03vd!`](@ref)) and `MB03VY`  (see [`PeriodicMatrices.SLICOTtools.mb03vy!`](@ref)).
"""
...
(see [`PeriodicMatrices.SLICOTtools.mb03vw!`](@ref)) and `MB03BD`  (see [`PeriodicMatrices.SLICOTtools.mb03bd!`](@ref)), 
based on algorithms proposed in [1] and [2].
   
The function `pschur1` is based on wrappers for the SLICOT subroutines `MB03VD`  (see [`PeriodicMatrices.SLICOTtools.mb03vd!`](@ref)), 
`MB03VY`  (see [`PeriodicMatrices.SLICOTtools.mb03vy!`](@ref))  and `MB03BD`  (see [`PeriodicMatrices.SLICOTtools.mb03bd!`](@ref)), 
based on algorithms proposed in [1] and [2].

The function `pschur2` is based on wrappers for the SLICOT subroutines `MB03VD`  (see [`PeriodicMatrices.SLICOTtools.mb03vd!`](@ref)), 
`MB03VY`  (see [`PeriodicMatrices.SLICOTtools.mb03vy!`](@ref)) and `MB03VW`  (see [`PeriodicMatrices.SLICOTtools.mb03vw!`](@ref)), 
...
The matrices `A(1)`, `...`, `A(p)` are contained in the `n×n×p` array `A` 
such that the `i`-th matrix `A(i)` is contained in `A[:,:,i]`. 
A range `ilh = (ilo, ihi)` can be specified, such that
all matrices `A(j), j = 1, ..., p`, are already in periodic Schur forms in rows and columns `1:ilo-1` and `ihi+1:n`, where
`n` is the first dimension of `A`.
The resulting reduced matrices `S(1)`, `...`, `S(p)`, representing the periodic Schur decimposition 
overwrite the input matrices `A(1)`, `...`, `A(p)`.
If `withZ = true`, then `Z` must contain on input an `n×n×p` array, which will be overwritten on output
with the orthogonal matrices `Z(1)`, `...`, `Z(p)` used for reduction. 
If `withZ = false`, orthogonal transformations are not computed and `Z` can be an arbiitrary three-dimensional array (e.g., an empty `0×0×0` array). 
The resulting reduced matrices `S(1)`, `...`, `S(p)` and the orthogonal transformation matrices `Z(1)`, `...`, `Z(p)`, 
...
To reduce the number of allocations when the periodic Schur decomposition needs to be repeteadly computed, 
the internally needed workspace can be pre-allocated in a tuple `wspace` using the function [`PeriodicMatrices.ws_pschur`](@ref)). 
"""
...
"""
function ws_pschur(A::AbstractArray{Float64,3})
   n, p = size(A,1),size(A,3)
...
The function `pschur` is based on wrappers for the SLICOT subroutines `MB03VW` (see [`PeriodicMatrices.SLICOTtools.mb03vw!`](@ref))
 and `MB03BD` (see [`PeriodicMatrices.SLICOTtools.mb03bd!`](@ref)), 
based on algorithms proposed in [1] and [2].
