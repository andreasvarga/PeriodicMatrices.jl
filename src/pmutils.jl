...
the characteristic multipliers `cm` are the eigenvalues of 
...
`A` is given as a `PeriodicArray`. The characteristic multipliers are computed  
as the eigenvalues of the reverse product of component matrices,  without evaluating the product. 
...
Compute the characteristic multipliers of a discrete-time periodic matrix `A(t)`. 
`A` is given as a `PeriodicMatrix`. The characteristic multipliers are computed  
as the eigenvalues of the reverse product of component matrices,  without evaluating the product. 
The argument `k` specifies the starting index of component matrices (default: `k = 1`). 
If `fast = false` (default) then the eigenvalues are computed using an approach
based on the periodic Schur decomposition [1], while if `fast = true` 
the structure exploiting reduction [2] of an appropriate lifted pencil is employed. 
This later option may occasionally lead to inaccurate results for large number of matrices. 

_Note:_ The first `nmin` components of `cm` contains the `core characteristic multipliers`,
where `nmin` is the minimum row dimensions of component matrices, 
while the last `ncur-nmin` components of `cm` are zero, 
where `ncur` is the column dimension of the `k`-th component matrix. 
...
#    return A[k]
# end
# function getpm(A::SPM, k, dperiod::Union{Int,Missing} = missing) where {SPM <: SwitchingPeriodicMatrix}
#    return A[k]
# end
# function getpm(A::SPM, k, dperiod::Union{Int,Missing} = missing) where {SPM <: SwitchingPeriodicArray}
#    # i = ismissing(dperiod) ? mod(k-1,A.dperiod)+1 : mod(k-1,dperiod)+1
#    return A[k]
#    #return view(A.M,i)
# end
...
"""
function pmcopy(A::PeriodicMatrix)
    PeriodicMatrix(A.M,A.period;nperiod = A.nperiod)
end
