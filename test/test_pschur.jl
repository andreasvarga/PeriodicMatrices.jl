...
ev = eigvals(inv(E3)*A3*inv(E2)*A2*inv(E1)*A1);
As, Qt, evp, sind, α, γ = PeriodicMatrices.pgschur!(reshape([A1 E1 A2 E2 A3 E3],5,5,6), [trues(3) falses(3)]'[:]; rev=true, schurindex = 2); evp
@test sort(real(ev)) ≈ sort(real(evp)) && sort(imag(ev)) ≈ sort(imag(evp))
...
As, Qt, evp, sind, α, γ = PeriodicMatrices.pgschur(A, [trues(3) falses(3)]'[:]; rev=true); evp
@test sort(real(ev)) ≈ sort(real(evp)) && sort(imag(ev)) ≈ sort(imag(evp))
@test Qt[:,:,2]'*A[:,:,1]*Qt[:,:,1] ≈ As[:,:,1] && Qt[:,:,2]'*A[:,:,2]*Qt[:,:,3] ≈ As[:,:,2] && 
      Qt[:,:,4]'*A[:,:,3]*Qt[:,:,3] ≈ As[:,:,3] && Qt[:,:,4]'*A[:,:,4]*Qt[:,:,5] ≈ As[:,:,4] && 
      Qt[:,:,6]'*A[:,:,5]*Qt[:,:,5] ≈ As[:,:,5] && Qt[:,:,6]'*A[:,:,6]*Qt[:,:,1] ≈ As[:,:,6]
...
select = real(evp) .< 0
PeriodicMatrices.pgordschur!(As, [trues(3) falses(3)]'[:], Qt, select; rev=true, schurindex = sind); 
@test Qt[:,:,2]'*A[:,:,1]*Qt[:,:,1] ≈ As[:,:,1] && Qt[:,:,2]'*A[:,:,2]*Qt[:,:,3] ≈ As[:,:,2] && 
      Qt[:,:,4]'*A[:,:,3]*Qt[:,:,3] ≈ As[:,:,3] && Qt[:,:,4]'*A[:,:,4]*Qt[:,:,5] ≈ As[:,:,4] && 
      Qt[:,:,6]'*A[:,:,5]*Qt[:,:,5] ≈ As[:,:,5] && Qt[:,:,6]'*A[:,:,6]*Qt[:,:,1] ≈ As[:,:,6]
...
As, Qt, evp1, sind1, α1, γ1 = PeriodicMatrices.pgschur!(reshape([A1 E1 A2 E2 A3 E3],5,5,6), [trues(3) falses(3)]'[:]; rev=false); evp1
@test sort(real(ev1)) ≈ sort(real(evp1)) && sort(imag(ev1)) ≈ sort(imag(evp1))
...
select = abs.(evp1) .< 1
PeriodicMatrices.pgordschur!(As, [trues(3) falses(3)]'[:], Qt, select; rev=false, schurindex = sind1); 
@test Qt[:,:,1]'*A[:,:,1]*Qt[:,:,2] ≈ As[:,:,1] && Qt[:,:,3]'*A[:,:,2]*Qt[:,:,2] ≈ As[:,:,2] && 
      Qt[:,:,3]'*A[:,:,3]*Qt[:,:,4] ≈ As[:,:,3] && Qt[:,:,5]'*A[:,:,4]*Qt[:,:,4] ≈ As[:,:,4] && 
      Qt[:,:,5]'*A[:,:,5]*Qt[:,:,6] ≈ As[:,:,5] && Qt[:,:,1]'*A[:,:,6]*Qt[:,:,6] ≈ As[:,:,6]
...
ev = eigvals(A1*inv(E1)*A2*inv(E2)*A3*inv(E3))
As, Qt, evp, sind, α, γ = PeriodicMatrices.pgschur(A,[trues(3) falses(3)]'[:]; rev=false); evp
@test sort(real(ev)) ≈ sort(real(evp)) && sort(imag(ev)) ≈ sort(imag(evp))
@test Qt[1]'*A[1]*Qt[2] ≈ As[1] && Qt[3]'*A[2]*Qt[2] ≈ As[2] && 
      Qt[3]'*A[3]*Qt[4] ≈ As[3] && Qt[5]'*A[4]*Qt[4] ≈ As[4] && 
      Qt[5]'*A[5]*Qt[6] ≈ As[5] && Qt[1]'*A[6]*Qt[6] ≈ As[6]
...
select = abs.(evp) .< 1
PeriodicMatrices.pgordschur!(As, [trues(3) falses(3)]'[:], Qt, select; rev=false, schurindex = sind); 
@test Qt[1]'*A[1]*Qt[2] ≈ As[1] && Qt[3]'*A[2]*Qt[2] ≈ As[2] && 
      Qt[3]'*A[3]*Qt[4] ≈ As[3] && Qt[5]'*A[4]*Qt[4] ≈ As[4] && 
      Qt[5]'*A[5]*Qt[6] ≈ As[5] && Qt[1]'*A[6]*Qt[6] ≈ As[6]
evp2 = ordeigvals(As[1]*inv(As[2])*As[3]*inv(As[4])*As[5]*inv(As[6]))
